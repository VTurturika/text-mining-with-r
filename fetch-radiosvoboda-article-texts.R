...
input_file <- "radiosvoboda_article_links.txt"
output_file <- "radiosvoboda_article_texts.csv"
...
for (i in seq(start, end)) {
  ...
  title <- html_text2(html_node(web_content, css = ".title"))
  text <- html_text2(html_nodes(web_content, css = "#article-content p"))
  text <- paste(text, collapse = " ")
  text <- str_squish(text)
  
  # Check if text content is empty
  if (text == "") {
    cat("Skipping link", i, "due to empty text content\n")
    next
  }
