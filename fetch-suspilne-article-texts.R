...
input_file <- "suspilne_article_links.txt"
output_file <- "suspilne_article_texts.csv"
...
for (i in seq(start, end)) {
  ...
  date_published = substr(html_attr(html_node(web_content, css = ".c-article-info-share__info time"), "datetime"), 1, 10)
  title <- html_text2(html_node(web_content, css = ".c-article-headline h1"))
  text <- html_text2(html_node(web_content, css = ".c-article-content"))
